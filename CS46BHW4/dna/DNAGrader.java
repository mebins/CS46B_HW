...
public class DNAGrader {
	...
	private enum Category {
		...
		DNARecord(10), FastqException(10), FastqRecord(18), FastaRecord(10), FastqReader(10), FastaWriter(
				10), FileConverter(24), Style(5), Comments(3);
	...
	private void gradeFastqRecord() {
		...
		// Does class have (String, String, String) ctor that throws
		// FastqException?
		...
		try {
			...
			deduct(Category.FastqRecord,
					"FastqRecord ctor did not throw exception on invalid input (>Rec1, AAAA, XXXX)", 3);
		} catch (FastqException x) {
			// Should get here.
		}
		...
		if (rec1AX == null || rec2AX == null || rec1CX == null || rec1AY == null || rec2TY == null) {
			err = "FastqRecord ctor is incorrect, can't create test instances to test" + "equals() or compareTo()";
	...
	private void gradeConverter() {
		...
		// Check output. All records should be fasta (2 lines), with no
		// duplicates. Don't check
		// anything else.
		Set<String> observedDeflines = new HashSet<>();
		try (FileReader fr = new FileReader(fasta); BufferedReader br = new BufferedReader(fr);) {
			String defline;
